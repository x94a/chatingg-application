...
def encrypt(key, source, encode=True):
    ...
    padding = AES.block_size - len(source) % AES.block_size  # calculate needed padding.blink
    source += bytes([padding]) * padding
    data = IV + encryptor.encrypt(source)  # save IV first then encrypt . blink
    return base64.b64encode(data).decode("latin-1") if encode else data

def decrypt(key, source, decode=True):
    if decode:
        source = base64.b64decode(source.encode("latin-1"))
    key = SHA256.new(key).digest()  # use SHA-256 over our key to get a proper-sized AES key.blink
    IV = source[:AES.block_size]  # extract the IV from the beginning
    decryptor = AES.new(key, AES.MODE_CBC, IV)
    data = decryptor.decrypt(source[AES.block_size:])  # decrypt
    padding = data[-1]  # pick the padding value from the end.blink
...
class Login(Tk):
    def __init__(self):
        ...
        asad = ImageTk.PhotoImage(Image.open(resource_path('images/asad.png')))
        self.tk.call('wm', 'iconphoto', self._w, asad)
        ...
        self.options['port'].set(5555)

        if platform.system() == 'Darwin':
            photo = ImageTk.PhotoImage(Image.open(resource_path('images/asad.png')))
        else:
            photo = PhotoImage(file='images/asad.png')
        #photo = photo.zoom(2)
    ...
    def register_user(self):
        ...
        self.reg.destroy()
#dont try steal my source.. i will find you then i will patch you <3 BLINK AMEBA IS HERE BAE <3 <3
class MainWindow(Tk):
    def __init__(self, username, host, port, server_pwd):
        ...
        user = username
        key = server_pwd
        key = key.encode('utf-8') # encode to type bytes \ to make it easy and decrease  time .. <3
        h = host
        p = port
        ...
        self.options['chatbox'] = Text(self, foreground="deeppink", background="black", highlightcolor="black", highlightbackground="black", height = 28, width = 80)
